    ...
    do while(CurrentTime .le. EndTime)
        ...
        if(USF) then
            ...
            if (StrainApprox) then
                call StrainApproximationByNode()
            end if
            call ParticleStressUpdate() ! Eq.(3.49-3.50 ...)
            if (StressApprox) then
                call StressApproximationByNode()
            end if
        end if
        ...
        call GridVelocityAccelerationUpdate()        
        ...
        call ParticleVelocityPositionUpdate()
        ...
        if(.NOT. USF) then
            ...
            if (StrainApprox) then
                !call StrainApproximationByNode()
                call StrainIncrementApproximationByNode()
            end if
			!call StressApproximationByNode()
			!call particlepwp()
            ...
            if (StressApprox) then
                !call StressApproximationByNodeQUAD()
                call StressApproximationByNode()
            end if
            call ParticlePositionUpdateOnly()
        end if
        ...
        ! report at specified time
        if (CurrentTime > PreOutTime(iprint)) then
            iprint = iprint + 1
            timeFlag = .true.
            call OutPrint(timeFlag)
        ...
        if (CurrentTime.ge.prt) then
            ...
            write(iomsg,100) istep, CurrentTime, EngKinetic, &
                EngKinetic+EngInternal
100             format(1x, "Step=", i10, 1p, "  T=", e10.3, &
                "  K.E.=", e10.3, "  T.E.=", e10.3)

            write(*,300) Momentum(:)
            write(iomsg,300) Momentum(:)
...
203 format("** Total elapsed time : ", f12.4, " seconds")
