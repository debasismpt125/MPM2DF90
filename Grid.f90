    ...
    type GridNode
        ...
        logical:: Fric(3)   ! frictional boundary
    end type GridNode
    ...
    type GridNodeProperty
        ...
        real(8):: AXg(3)    ! acceleration
        real(8):: VXg(3)    ! velocity
        real(8):: VXg_old(3)! old velocity
    ...
    real(8),parameter:: SHPC(8) = (/0.125d0, 0.125d0, 0.125d0, 0.125d0, &
        0.125d0, 0.125d0, 0.125d0, 0.125d0/)
    real(8):: DNDXC(8), DNDYC(8), DNDZC(8)
    ...
    do iz = 1, NGz
        do iy = 1, NGy
            do ix = 1, NGx
                ...
                ! frictional surface iz = 1 
                if ((iz==1.and.FixS(5)==3)) then
                    node_list(i)%Fric(3) = .true.
                    node_list(i)%Fix_z = .true.
                end if
            end do
        end do
    ...
    if (ider .NE. 1) pt%SHP(1:8) = sx * sy * sz * 0.125d0

    if (ider .NE. 0) then
        pt%DNDX(1:8) = SNX * sy * sz * iJacobi4
        pt%DNDY(1:8) = SNY * sx * sz * iJacobi4
        pt%DNDZ(1:8) = SNZ * sx * sy * iJacobi4
    end if
    ...
    integer, intent(in):: p
    integer:: i, j, nb_InflNode, InflNode(27)
    real(8):: sh(3), dn(3), r(3)
    real(8):: rpg(27,3)
    type(Particle), POINTER :: pt
    ...
    do i = 1, nb_InflNode
        ...
        pt%SHP(i) = sh(1)*sh(2)*sh(3)
        pt%DNDX(i) = dn(1)*sh(2)*sh(3)*iDCell
        pt%DNDY(i) = sh(1)*dn(2)*sh(3)*iDCell
        pt%DNDZ(i) = sh(1)*sh(2)*dn(3)*iDCell
    end do
    ...
    integer:: InflNode(27), nb_InflNode, i
    real(8):: rpg(27,3), x(3) ! nature coordinate ( -1 < x,y,z < 1 )
    type(Particle), POINTER :: pt
    ...
    implicit none
    integer:: n, inode, ix, iy, iz ! loop counter
    ...
    do iz = 1, NumCellz
        do ix = 1, NumCellx
            ...
            do iy = NumCelly, 1, -1
                icell = (iz - 1)*NumCellxy + (iy - 1)*NumCellx + ix
                gc => cell_list(icell)
                ...
                if (gc%InOutProp > 0 .and. surfaceCellNotFound) then
                    ...
                    grid_list(inode, comID)%InOutProp = 2
                    inode = CellsNode(icell, 7)
                    grid_list(inode, comID)%InOutProp = 2
                    inode = CellsNode(icell, 8)
                    grid_list(inode, comID)%InOutProp = 2
                end if
            end do
        end do
